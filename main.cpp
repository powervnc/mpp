...
class Matrix {
    ...
    public:
        static Matrix readFromFile(size_t dim, const std::string& filename) {
            ...
            fin.read(reinterpret_cast<char*>(mat.data_.data()),
     ...
     static Matrix readBinaryParallel(size_t dim, const std::string& filename, size_t num_threads) {
        ...
        auto worker = [&](size_t start, size_t end) {
            ...
            fin.read(reinterpret_cast<char*>(&mat.data_[start]),
    ...
    Matrix getTransposed() const {
     ...
     static Matrix multiplyParallel(const Matrix& X, const Matrix& Y, size_t num_threads) {
        ...
        std::vector<std::thread> threads;
        size_t rows_per_thread = X.size_ / num_threads;
        ...
        auto worker = [&](size_t start, size_t end) {
            for (size_t r = start; r < end; ++r) {
                ...
                for (size_t c = 0; c < X.size_; ++c) {
                    ...
                    result.data_[r * X.size_ + c] =
                        std::inner_product(rowX, rowX + X.size_, rowYt, 0.0);
                }
            }
        };
        ...
        size_t start = 0;
        for (size_t t = 0; t < num_threads; ++t) {
            size_t end = (t == num_threads - 1) ? X.size_ : start + rows_per_thread;
            threads.emplace_back(worker, start, end);
            start = end;
        }
